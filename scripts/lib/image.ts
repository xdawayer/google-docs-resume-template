 ...
 */
export async function writeMaster(pngBuffer: Buffer, outPath: string, targetWidth = 1600): Promise<MasterResult> {
  const img = sharp(pngBuffer).flatten({ background: "#ffffff" }).resize({ width: targetWidth, withoutEnlargement: true });
